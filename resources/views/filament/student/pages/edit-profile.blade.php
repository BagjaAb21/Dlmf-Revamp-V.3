...
@php
    ...
    $sh     = "display:flex;align-items:center;gap:0.5rem;padding:0.875rem 1.25rem;border-bottom:1px solid rgba(124,58,237,0.08);background:linear-gradient(135deg,#F9F7FF,#F3F0FF);";
    ...
    $icon   = "width:36px;height:36px;min-width:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;";
    $pLabel  = "font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#94A3B8;margin-bottom:0.1rem;";
...
">
    <div style="position:absolute;top:-30px;right:-30px;width:150px;height:150px;background:rgba(255,255,255,0.04);border-radius:50%;pointer-events:none;"></div>
    <div style="width:50px;height:50px;min-width:50px;background:rgba(255,255,255,0.13);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem;border:1.5px solid rgba(255,255,255,0.2);position:relative;z-index:1;">👤</div>
    <div style="position:relative;z-index:1;">
        <h1 style="font-family:'Poppins',sans-serif;font-size:clamp(1.1rem,3vw,1.35rem);font-weight:800;color:#fff;margin:0 0 0.2rem;line-height:1.25;">
...
<div style="{{ $card }}">
    ...
    <div style="padding:1.25rem;display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem;">
        ...
        <div style="{{ $infoRow }}">
            ...
            <div><div style="{{ $pLabel }}">Nama Lengkap</div><div style="{{ $pValue }}">{{ $user->name }}</div></div>
        </div>

        <div style="{{ $infoRow }}">
            <div style="{{ $icon }} background:linear-gradient(135deg,#06B6D4,#22D3EE);">✉️</div>
            <div><div style="{{ $pLabel }}">Email</div><div style="{{ $pValue }} word-break:break-all;">{{ $user->email }}</div></div>
        </div>

        <div style="{{ $infoRow }}">
            <div style="{{ $icon }} background:linear-gradient(135deg,#10B981,#34D399);">📱</div>
            <div><div style="{{ $pLabel }}">Nomor HP</div><div style="{{ $pValue }}">{{ $user->phone ?: '—' }}</div></div>
        </div>

        <div style="{{ $infoRow }}">
            <div style="{{ $icon }} background:linear-gradient(135deg,#7C3AED,#C084FC);">🎓</div>
            <div><div style="{{ $pLabel }}">Level Bahasa Jerman</div><div style="{{ $pValue }}">{{ $levelLabel ?: '—' }}</div></div>
        </div>

        <div style="{{ $infoRow }}">
            <div style="{{ $icon }} background:linear-gradient(135deg,#F59E0B,#FBBF24);">🕐</div>
            <div><div style="{{ $pLabel }}">Jadwal Kelas</div><div style="{{ $pValue }}">{{ $scheduleLabel ?: '—' }}</div></div>
        </div>
...
<div style="{{ $card }}">
    <div style="{{ $sh }} justify-content:space-between;">
        <div style="display:flex;align-items:center;gap:0.5rem;">
            ...
            <span style="background:#7C3AED;color:#fff;font-size:0.65rem;font-weight:700;padding:0.1rem 0.5rem;border-radius:9999px;">
                {{ $enrollments->count() }}
            </span>
        </div>
            ...
            @foreach($enrollments as $enr)
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem;background:#F9FAFB;border:1px solid rgba(124,58,237,0.07);border-radius:12px;padding:0.875rem 1rem;transition:background 0.15s;"
                    ...
                    <div style="display:flex;align-items:center;gap:0.75rem;flex:1;min-width:0;">
                        <div style="width:38px;height:38px;min-width:38px;background:linear-gradient(135deg,#7C3AED,#A855F7);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;">📖</div>
                        <div style="min-width:0;">
                            <div style="font-size:0.875rem;font-weight:700;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                {{ $enr->product?->name ?? '—' }}
                            </div>
                    ...
                    <span style="font-size:0.7rem;font-weight:700;background:{{ $sc[0] }};color:{{ $sc[1] }};border:1px solid {{ $sc[2] }};padding:0.2rem 0.65rem;border-radius:9999px;white-space:nowrap;">
                        {{ $sc[3] }}
                    </span>
                </div>
            @endforeach
...
<div style="display:flex;align-items:flex-start;gap:0.75rem;background:#F5F3FF;border:1px solid rgba(124,58,237,0.15);border-left:4px solid #7C3AED;border-radius:12px;padding:0.875rem 1.1rem;">
    ...
    <p style="margin:0;font-size:0.82rem;font-weight:500;color:#5B21B6;line-height:1.6;">
        <strong>Tips:</strong> Isi <strong>Jadwal, Level</strong>, dan <strong>Kota</strong> — data ini digunakan admin untuk mengatur kelasmu.
    </p>
</div>
...
<x-filament-panels::form wire:submit="save">
    ...
    <div style="display:flex;justify-content:flex-end;align-items:center;gap:0.75rem;padding-top:0.25rem;flex-wrap:wrap;">

        <a href="{{ \App\Filament\Student\Pages\Dashboard::getUrl() }}"
