...
@php
    ...
    $card = "background:#fff;border-radius:16px;border:1px solid rgba(124,58,237,0.09);box-shadow:0 2px 12px rgba(124,58,237,0.07);overflow:hidden;";
    $sectionHead = "display:flex;align-items:center;gap:0.5rem;padding:0.875rem 1.25rem;border-bottom:1px solid rgba(124,58,237,0.08);background:linear-gradient(135deg,#F9F7FF,#F3F0FF);";
    $sectionLabel = "font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#5B21B6;";
@endphp
...
">
    ...
    <div style="position:absolute;bottom:-40px;right:80px;width:130px;height:130px;background:rgba(255,255,255,0.04);border-radius:50%;pointer-events:none;"></div>
    ...
    <div style="position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
        <div>
            <span style="display:inline-block;background:rgba(255,255,255,0.12);color:#FDE047;font-size:0.65rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;padding:0.2rem 0.75rem;border-radius:9999px;border:1px solid rgba(253,224,71,0.25);margin-bottom:0.6rem;">
                🇩🇪 Area Siswa — DlmF
            </span>
            <h1 style="font-family:'Poppins',sans-serif;font-size:clamp(1.25rem,3vw,1.65rem);font-weight:800;color:#fff;margin:0 0 0.3rem;line-height:1.25;">
...
<div style="display:flex;align-items:flex-start;gap:0.875rem;background:#FFFBEB;border:1.5px solid #F59E0B;border-left-width:4px;border-radius:14px;padding:1rem 1.25rem;box-shadow:0 2px 12px rgba(245,158,11,0.1);">
    <div style="width:34px;height:34px;min-width:34px;border-radius:50%;background:#F59E0B;display:flex;align-items:center;justify-content:center;font-size:0.9rem;">⚠️</div>
    <div>
        ...
        <p style="font-size:0.8rem;color:#78350F;margin:0 0 0.5rem;line-height:1.5;">
            Isi <strong>Level, Jadwal</strong>, dan <strong>Kota</strong> agar admin dapat memproses kelasmu lebih cepat.
        </p>
        <a href="{{ \App\Filament\Student\Pages\EditProfile::getUrl() }}"
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem;">
    ...
    <div style="{{ $card }} transition:transform 0.2s,box-shadow 0.2s;"
        ...
        <div style="padding:1rem 1.1rem;">
            ...
            <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#64748B;margin-bottom:0.2rem;">Level Saya</div>
    ...
    <div style="{{ $card }} transition:transform 0.2s,box-shadow 0.2s;"
        ...
        <div style="padding:1rem 1.1rem;">
            <div style="font-size:1.5rem;line-height:1;margin-bottom:0.5rem;">🕐</div>
            <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#64748B;margin-bottom:0.2rem;">Jadwal Kelas</div>
    ...
    <div style="{{ $card }} transition:transform 0.2s,box-shadow 0.2s;"
        ...
        <div style="padding:1rem 1.1rem;">
            <div style="font-size:1.5rem;line-height:1;margin-bottom:0.5rem;">📖</div>
            <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#64748B;margin-bottom:0.2rem;">Kelas Aktif</div>
            ...
            <div style="font-size:0.72rem;color:#94A3B8;margin-top:0.15rem;">dari {{ $totalCount }} terdaftar</div>
        </div>
    </div>
    ...
    <div style="{{ $card }} transition:transform 0.2s,box-shadow 0.2s;"
        ...
        <div style="padding:1rem 1.1rem;">
            <div style="font-size:1.5rem;line-height:1;margin-bottom:0.5rem;">👤</div>
            <div style="font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#64748B;margin-bottom:0.2rem;">Status Profil</div>
            @if($isProfileComplete)
...
<div style="background:#fff;border-radius:16px;border:1px solid rgba(124,58,237,0.09);box-shadow:0 2px 12px rgba(124,58,237,0.07);overflow:hidden;">

    {{-- Header --}}
    <div style="display:flex;align-items:center;justify-content:space-between;padding:0.875rem 1.25rem;border-bottom:1px solid rgba(124,58,237,0.08);background:linear-gradient(135deg,#F9F7FF,#F3F0FF);">
        <div style="display:flex;align-items:center;gap:0.5rem;">
            <span>📚</span>
            <span style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#5B21B6;">Kelas Terbaru</span>
            @if($totalCount > 0)
                <span style="background:#7C3AED;color:#fff;font-size:0.65rem;font-weight:700;padding:0.1rem 0.5rem;border-radius:9999px;">{{ $totalCount }}</span>
            @endif
        </div>
        <a href="{{ \App\Filament\Student\Pages\MyClasses::getUrl() }}"
    ...
    @if($recentEnrollments->isEmpty())
        <div style="padding:2.5rem;text-align:center;">
            ...
            <p style="font-size:0.875rem;color:#64748B;margin:0 0 1rem;font-weight:500;">Belum ada kelas terdaftar.</p>
            <a href="/harga"
            ...
            @foreach($recentEnrollments as $enr)
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;gap:0.75rem;background:#F9FAFB;border:1px solid rgba(124,58,237,0.07);border-radius:12px;padding:0.75rem 1rem;transition:background 0.15s;"
                    ...
                    <div style="display:flex;align-items:center;gap:0.75rem;min-width:0;flex:1;">
                        <div style="width:36px;height:36px;min-width:36px;background:linear-gradient(135deg,#7C3AED,#A855F7);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:0.9rem;">📖</div>
                        <div style="min-width:0;">
                            <div style="font-size:0.875rem;font-weight:700;color:#111827;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                {{ $enr->product?->name ?? '—' }}
                            </div>
                    ...
                    <span style="font-size:0.7rem;font-weight:700;background:{{ $sc[0] }};color:{{ $sc[1] }};border:1px solid {{ $sc[2] }};padding:0.2rem 0.6rem;border-radius:9999px;white-space:nowrap;flex-shrink:0;">
                        {{ $sc[3] }}
                    </span>
                </div>
            @endforeach
